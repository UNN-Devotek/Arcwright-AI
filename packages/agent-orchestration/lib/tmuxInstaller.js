'use strict';
/**
 * @arcwright-ai/tmux — lib/tmuxInstaller.js
...
/** Detect platform — respects ARCWRIGHT_PLATFORM env override for testing */
function detectPlatform() {
  if (process.env.ARCWRIGHT_PLATFORM) return process.env.ARCWRIGHT_PLATFORM;
...
async function setupTmux(projectRoot, chalk, platform) {
  ...
  if (!await fs.pathExists(tmuxSrc)) {
    console.log(chalk.red('\n  ✗  tmux source files not found in package. Try reinstalling: npx @arcwright-ai/agent-orchestration@latest'));
    rl.close();
    return;
  }
...
async function writeGlobalClaudeMd(chalk) {
  ...
  const START = '<!-- arcwright-tmux-start -->';
  const END   = '<!-- arcwright-tmux-end -->';
  const block = `${START}\n${tmuxSection}\n${END}`;
...
/**
 * One-time migration: strip old markerless Arcwright/tmux sections from pre-1.0.19 installers.
 */
async function migrateOldClaudeMd(filePath) {
  if (!await fs.pathExists(filePath)) return;
  const content = await fs.readFile(filePath, 'utf8');
  if (content.includes('<!-- arcwright-tmux-start -->')) return;
