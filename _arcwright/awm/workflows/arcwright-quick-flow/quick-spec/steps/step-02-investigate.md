...
> 💡 **Agile note — why we do this:** Step 2 maps the *exact terrain* of the codebase before writing a single line of the spec. Skipping investigation is how teams end up implementing the right feature in the wrong place — fixing it after the fact costs 3× the effort.

**Report to user and proceed automatically:**

```
✅ Step 2 complete — technical context mapped.

📝 Moving to Step 3: Generate Spec
I'll translate everything we found into an implementation-ready plan.

Optional tools available any time:
  [A] Advanced Elicitation — structured discovery to surface hidden constraints
  [P] Party Mode Review — multi-agent critique of the investigation findings
  [AR] Adversarial Review — challenge the technical decisions before we write the spec
Type one of these to use them now, otherwise I'm proceeding.
```

#### Auto-Proceed Rules:

- Proceed immediately to Step 3 unless the user types `[A]`, `[P]`, or `[AR]` in response
- If `[A]`: Read fully and follow `{advanced_elicitation}` with current tech-spec content, process enhanced insights, ask user "Accept improvements? (y/n)", if yes update WIP file then proceed to Step 3
- If `[P]`: Read fully and follow `{party_mode_exec}` with current tech-spec content, process collaborative insights, ask user "Accept changes? (y/n)", if yes update WIP file then proceed to Step 3
- If `[AR]`: Execute `{project-root}/_arcwright/core/tasks/review-adversarial-general.xml` with the current WIP file, display findings, then proceed to Step 3
