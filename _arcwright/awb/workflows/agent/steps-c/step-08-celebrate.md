...
outputFile: {awb_creations_output_folder}/agent-completion-{agent_name}.md

# Task References
advancedElicitationTask: '{project-root}/_arcwright/core/workflows/advanced-elicitation/workflow.xml'
partyModeWorkflow: '{project-root}/_arcwright/core/workflows/party-mode/workflow.md'
installationDocs: 'https://github.com/arcwright-code-org/Arcwright-METHOD/blob/main/docs/modules/awb-arcwright-builder/custom-content-installation.md#standalone-content-agents-workflows-tasks-tools-templates-prompts'
validationWorkflow: '{project-root}/src/modules/awb/workflows/agent/steps-v/v-01-load-review.md'
---
...
"To make your agent installable and sharable, you'll need to package it as a standalone Arcwright content module. Here's what you need to know:"
...
3. **Copy your agent:** Copy the entire folder from `_arcwright-creations/{agent-name}/` to `agents/`
...
my-custom-stuff/
├── module.yaml
├── agents/                              # Copy entire folder from _arcwright-creations/
│   └── {agent-name}/
...
- **New projects:** The Arcwright installer will prompt for local custom modules
- **Existing projects:** Use "Modify Arcwright Installation" to add your module
...
📖 **[Arcwright Custom Content Installation Guide]({installationDocs})**
...
3. Copy entire agent folder from `_arcwright-creations/{agent-name}/` to `agents/`
4. Install via Arcwright installer
```
...
**Final Achievement:**
"You've successfully created a custom Arcwright agent from concept to installation-ready configuration. The journey from idea to deployable agent is complete!"
