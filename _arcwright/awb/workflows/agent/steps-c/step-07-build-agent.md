...
agentPlan: '{awb_creations_output_folder}/agent-plan-{agent_name}.md'

# Output paths (determined by hasSidecar)
agentBuildOutput: '{awb_creations_output_folder}/{agent-name}/'
agentYamlOutput: '{awb_creations_output_folder}/{agent-name}/{agent-name}.agent.yaml'
agentYamlOutputNoSidecar: '{awb_creations_output_folder}/{agent-name}.agent.yaml'
sidecarOutput: '{awb_creations_output_folder}/{agent-name}/{agent-name}-sidecar/'
...
critical-actions:
  - name: '{action-name}'
    ...
    sidecar-files:
      - '{project-root}/_arcwright/_memory/{sidecar-folder}/{file1}.md'
      - '{project-root}/_arcwright/_memory/{sidecar-folder}/{file2}.md'
...
metadata:
  sidecar-folder: '{sidecar-folder-name}'
  sidecar-path: '{project-root}/_arcwright/_memory/{sidecar-folder}/'
  hasSidecar: true
  agent-type: 'agent'
...
3. **Add README to Sidecar**:
   ...
   ## Runtime Access
   After Arcwright installation, this folder will be accessible at:
   `{project-root}/_arcwright/_memory/{sidecar-folder}/{filename}.md`
   ```
...
- IF AR: Execute {project-root}/_arcwright/core/tasks/review-adversarial-general.xml — pass the current editPlan (or plan document in context) as the content to review; display findings with recommendations; then redisplay this menu
