...
agent:
  metadata:
    id: _arcwright/agents/{agent-name}/{agent-name}.md
    name: 'Persona Name'
    title: 'Agent Title'
    icon: '<emoji>'
    module: stand-alone  # or awm, cis, bmgd
...
|-------|--------|---------|
| `id` | `_arcwright/agents/{name}/{name}.md` | `_arcwright/agents/commit-poet/commit-poet.md` |
| `name` | Persona name | `Inkwell Von Comitizen` |
| `title` | Role | `Commit Message Artisan` |
| `icon` | Single emoji | `📜` |
| `module` | `stand-alone` or module code | `awm`, `cis`, `bmgd` |
...
**Installation path:** `{project-root}/_arcwright/_memory/{sidecar-folder}/`

**ALL references MUST use:**
```yaml
{project-root}/_arcwright/_memory/{sidecar-folder}/{file}
```
...
# ✅ CORRECT
critical_actions:
  - "Load COMPLETE file {project-root}/_arcwright/_memory/journal-keeper-sidecar/memories.md"
  - "ONLY read/write files in {project-root}/_arcwright/_memory/journal-keeper-sidecar/"
...
```yaml
critical_actions:
  - 'Load COMPLETE file {project-root}/_arcwright/_memory/{sidecar-folder}/memories.md'
  - 'Load COMPLETE file {project-root}/_arcwright/_memory/{sidecar-folder}/instructions.md'
  - 'ONLY read/write files in {project-root}/_arcwright/_memory/{sidecar-folder}/'
```
...
menu:
  ...
  - trigger: SM or fuzzy match on save
    action: "Update {project-root}/_arcwright/_memory/{sidecar-folder}/memories.md"
...
- 'Load from {project-root}/_arcwright/_memory/{sidecar-folder}/knowledge/ but NEVER modify'
- 'Write ONLY to {project-root}/_arcwright/_memory/{sidecar-folder}/memories.md'
...
### hasSidecar: true
- [ ] ALL paths: `{project-root}/_arcwright/_memory/{sidecar-folder}/...`
