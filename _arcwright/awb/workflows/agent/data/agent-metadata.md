...
|----------|--------|-------|
| `id` | `_arcwright/agents/{agent-name}/{agent-name}.md` | Compiled output path; must match filename |
...
| `module` | `stand-alone`, `awm`, `cis`, `bmgd`, or custom | Lowercase, hyphenated for `stand-alone` |
...
### `id`
```yaml
id: _arcwright/agents/commit-poet/commit-poet.md
```
...
| `awm` | Business Management Module |
| `cis` | Continuous Innovation System |
| `bmgd` | Arcwright Game Development |
...
| What's the unique key? | `id` — `_arcwright/agents/{name}/{name}.md` |
...
id: _arcwright/agents/different-agent/different-agent.md

# ✅ Fix: match filename
id: _arcwright/agents/my-agent/my-agent.md
```
