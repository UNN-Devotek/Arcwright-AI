...
**What we're catching:**
- Content like: `Load {project-root}/_arcwright/foo/workflows/.../file.csv`
...
- **Output files using config variables:** Skip (won't exist yet - workflow not installed/running)
  ...
  - Example: `{awb_creations_output_folder}/file.md` - SKIP
...
1. Determines if current workflow is in a non-awb module
2. If yes, runs grep across all files to find awb-specific path assumptions
3. Returns all module awareness issues to parent for aggregation

```bash
# Check if in non-awb module, then search for awb-specific paths
if pwd | grep -q "/modules/[^/]\+/" && ! pwd | grep -q "/awb/"; then
  grep -rn "{project-root}/_arcwright/awb/" steps-c/ steps-e/ steps-v/ 2>/dev/null || echo "No awb-specific paths found"
fi
```
...
```json
{
  "known_config_variables": ["output_folder", "planning_artifacts", "awb_creations_output_folder", ...],
  "content_violations": [
  ...
  "module_awareness_issues": [
    {"file": "step-XX.md", "issue": "using awb-specific path in non-awb module"}
  ],
