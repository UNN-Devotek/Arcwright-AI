# Arcwright Workflow Template

This template provides the standard structure for all Arcwright workflow files. Copy and modify this template for each new workflow you create.
...
Load and read full config from {project-root}/_arcwright/[MODULE FOLDER]/config.yaml and resolve:
...
- `[MODULE FOLDER]` → Default is `core` unless this is for another module (such as awm, cis, or another as directed by user)
...
- `[CONFIG_PATH]` → Path to config file (usually `awm/config.yaml` or `awb/config.yaml`)
