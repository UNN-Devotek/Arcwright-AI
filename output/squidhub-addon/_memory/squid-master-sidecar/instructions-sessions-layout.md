...
{if session-state.platform == 'claude-code' or 'antigravity'}
  [2] Command blocks — each step prints a new-conversation command, auto-generated immediately.
      Context file written to _bmad-output/sessions/{session_id}/context.md. Best for: reviewing output between agents.
...
**Persistence file:** `_bmad-output/sessions/{session_id}/agent-sessions.md`
...
1. **CREATE** session directory `_bmad-output/sessions/{session_id}/` with subdirectories: `planning/`, `review/`, `stories/`, `implementation/`, `retros/`.
2. **CREATE** `_bmad-output/sessions/{session_id}/agent-sessions.md` with the step table pre-populated for the selected track (see Agent Session ID Persistence above).
3. **ALL OUTPUT goes into the session directory.** Never write artifacts to `_bmad-output/` root or scattered top-level folders. The session_id folder provides uniqueness — filenames inside are standard (no `{artifact_id}` suffix needed). **IMPORTANT:** Set `session_dir: _bmad-output/sessions/{session_id}` in each agent's handoff context block so downstream agents resolve paths correctly.
...
<context>
  ...
  session_dir: _bmad-output/sessions/{session_id}
  session_dir: _bmad-output/sessions/{session_id}
  ui_review_context:
    skill: ui-ux-pro-custom
    stack: shadcn
    styling_standards: docs/frontend/styling-standards.md
    output_path: _bmad-output/sessions/{session_id}/review/uv-findings.md
  ar_dry_context:
    skill: clean-code-standards
    agent: /bmad-agent-bmm-architect
    dry_output_path: _bmad-output/sessions/{session_id}/review/dry-findings.md
  sr_context:
    skills: [security-review, /security-review]
    agent: /bmad-agent-bmm-security
    output_path: _bmad-output/sessions/{session_id}/review/sr-findings.md
</context>
...
2. Update `_bmad-output/sessions/{session_id}/agent-sessions.md` — set that agent's row to `status: closed`, record the session ID and timestamp
...
Before printing the command, write a context file to `{project-root}/_bmad-output/sessions/{session_id}/context.md`:
...
Pre-output: Before generating any plan, spec, or document artifact, invoke writing-skills.
Planning artifacts: _bmad-output/sessions/{session_id}/planning/
...
# Context file: _bmad-output/sessions/{session_id}/context.md
...
The new conversation opens as that agent. The user can say "load the context file at `_bmad-output/sessions/{session_id}/context.md`" and the agent has everything it needs. After printing: halt and wait for user to confirm the step is complete before routing to the next step.
...
Generate `{project-root}/_bmad-output/sessions/{session_id}/start-{agent-slug}-{step}.ps1`:

```powershell
# Auto-generated by Squid-Master — self-deletes after run
# Branch: {branch}  |  Session: {session_id}  |  Step: {step-name}
# Context: _bmad-output/sessions/{session_id}/context.md
# ─────────────────────────────────────────────────────────────────────────────
...
$contextFile = "_bmad-output/sessions/{session_id}/context.md"
...
if ($NonInteractive) {
    ...
    $parsed.session_id | Out-File -FilePath "_bmad-output/sessions/last-session-id.txt" -NoNewline
...
$qsSessionId = Get-Content "_bmad-output/sessions/last-session-id.txt"
...
**Architecture Notes:** Brief, not a full Architecture Doc. Target ≤ 2 pages. Output to `_bmad-output/sessions/{session_id}/planning/arch-notes.md`. Contents:
...
4. Writes `sprint-plan.md` to `_bmad-output/sessions/{session_id}/planning/`
...
- Session directory already created at session init: `_bmad-output/sessions/{session_id}/`

**All artifacts use standard filenames inside the session directory — no `{artifact_id}` suffix needed (the session folder provides uniqueness):**

| Artifact | Path |
|---|---|
| Review plan | `sessions/{session_id}/planning/review-plan.md` |
| Review synthesis | `sessions/{session_id}/planning/review-synthesis.md` |
| Research synthesis | `sessions/{session_id}/planning/research-synthesis.md` |
| Research bucket N | `sessions/{session_id}/planning/research-{N}.md` |
| UI review findings | `sessions/{session_id}/review/uv-findings.md` |
| UI review loop pass N | `sessions/{session_id}/review/uv-findings-pass{N}.md` |
| DRY/SOLID review findings | `sessions/{session_id}/review/dry-findings.md` |
| DRY/SOLID loop pass N | `sessions/{session_id}/review/dry-findings-pass{N}.md` |
| Security review findings | `sessions/{session_id}/review/sr-findings.md` |
| Security review loop pass N | `sessions/{session_id}/review/sr-findings-pass{N}.md` |
| AR findings | `sessions/{session_id}/review/ar-findings.md` |
| AR loop pass N | `sessions/{session_id}/review/ar-findings-pass{N}.md` |
| Epic retro | `sessions/{session_id}/retros/epic-{N}-retro.md` |
| UX notes | `sessions/{session_id}/planning/ux-notes.md` |
| Arch notes | `sessions/{session_id}/planning/arch-notes.md` |
| PRD amends | `sessions/{session_id}/planning/prd-amends.md` |
| Quick spec | `sessions/{session_id}/planning/quick-spec.md` |
| PRD | `sessions/{session_id}/planning/prd.md` |
| UX design | `sessions/{session_id}/planning/ux-design.md` |
| Architecture | `sessions/{session_id}/planning/architecture.md` |
| Sprint plan | `sessions/{session_id}/planning/sprint-plan.md` |
| IR check | `sessions/{session_id}/planning/ir-check.md` |
| Tech spec | `sessions/{session_id}/implementation/tech-spec.md` |
| Story files | `sessions/{session_id}/stories/{epic}-{story}-{slug}.md` |
| Context handoff | `sessions/{session_id}/context.md` |
| Agent sessions | `sessions/{session_id}/agent-sessions.md` |

All paths are relative to `_bmad-output/`.
...
Dispatch in-process `analyst-agent` sub-agents per bucket — concurrently (Mode [4]) or sequentially (Mode [1]). Output files: `research-{N}.md`.

After researchers complete, synthesize into: `research-synthesis.md`
...
<context>
  ...
  artifact_path: {research-synthesis.md | review_target}
  track: review
  review_type: full
  session_dir: _bmad-output/sessions/{session_id}
  ui_review_context:
...
Categorize: `🔴 Critical` / `🟡 Major` / `🟢 Minor`. Write `review-synthesis.md`.

#### Step 6 — Sort Artifacts → Build Plan File

Write master plan file: `review-plan.md`
...
**Step 8L:** Route `/bmad-agent-bmm-sm` → `/bmad-bmm-create-epics-and-stories`. Input: `review-plan.md · review-synthesis.md · existing prd.md/ux-design.md if present for the target area`. 🔴 Critical findings take priority in story ordering.
...
**Step 14L:** Epic Retrospective — `/bmad-agent-bmm-sm` → `/bmad-bmm-retrospective`. Output: `epic-{N}-retro.md`.

#### SMALL PATH — Quick Dev

**Step 8S:** Route `/bmad-agent-bmm-quick-flow-solo-dev` → `/bmad-bmm-quick-dev` (split pane). Fixes ALL findings (🔴 + 🟡 + 🟢) from `review-plan.md`.
...
review_plan_path: _bmad-output/sessions/{session_id}/planning/review-plan.md
session_dir: _bmad-output/sessions/{session_id}
...
**Output:** `uv-findings.md` in `_bmad-output/sessions/{session_id}/review/`
...
ui_review_verdict:
  ...
  major: {count}
  minor: {count}
  findings_path: _bmad-output/sessions/{session_id}/review/uv-findings.md
...
All output goes to `_bmad-output/sessions/{session_id}/review/`. Run ALL passes without halting between them.
...
3. ux-designer writes `uv-findings-pass1.md` with findings and fix status
...
Re-run ux-designer to verify fixes landed. Agent re-reviews, fixes any regressions or remaining items. Write `uv-findings-pass2.md`.

**Pass 3** (if `uv_loop_max = 3` AND pass 2 had findings):
Re-run ux-designer. Final verification pass. Write `uv-findings-pass3.md`.

**After final pass OR early exit on 🟢-only:**
- Write canonical: `uv-findings.md` (copy of last pass file)
- Announce:
  ```
...
**Per-pass output files:** `uv-findings-pass{N}.md`
**Canonical output:** `uv-findings.md` (written after loop exits)
...
**Output:** `dry-findings.md` in `_bmad-output/sessions/{session_id}/review/`
...
dry_review_verdict:
  ...
  major: {count}
  minor: {count}
  findings_path: _bmad-output/sessions/{session_id}/review/dry-findings.md
  status: passed | needs_fixes
```
...
<context>
  ...
  session_dir: _bmad-output/sessions/{session_id}
  skill: clean-code-standards
  output_path: _bmad-output/sessions/{session_id}/review/dry-findings.md
  execution_directive: >
...
All output goes to `_bmad-output/sessions/{session_id}/review/`. Run ALL passes without halting.
...
3. architect-agent writes `dry-findings-pass1.md` with findings and fix status
4. Krakken reads results:
...
Re-run architect-agent to verify fixes landed. Agent re-reviews, fixes any regressions or remaining items. Write `dry-findings-pass2.md`.

**Pass 3** (if `dry_loop_max = 3` AND pass 2 had findings):
Re-run architect-agent. Final verification pass. Write `dry-findings-pass3.md`.

**After final pass OR early exit on 🟢-only:**
- Write canonical: `dry-findings.md` (copy of last pass)
...
**Per-pass output files:** `dry-findings-pass{N}.md`
**Canonical output:** `dry-findings.md` (written after loop exits)
...
**Output:** `sr-findings.md` in `_bmad-output/sessions/{session_id}/review/`
...
sr_review_verdict:
  ...
  minor: {count of LOW}
  findings_path: _bmad-output/sessions/{session_id}/review/sr-findings.md
  status: passed | needs_fixes
```
...
3. Agent writes `sr-findings-pass1.md` with findings and fix status
4. Krakken reads results:
...
Re-run security agent to verify fixes landed. Agent re-reviews, fixes any regressions or remaining items. Write `sr-findings-pass2.md`.

**Pass 3** (if `sr_loop_max = 3` AND pass 2 had findings):
Re-run security agent. Final verification pass. Write `sr-findings-pass3.md`.

**After final pass OR early exit on 🟢-only:**
- Write canonical: `sr-findings.md` (copy of last pass)
...
**Per-pass output files:** `sr-findings-pass{N}.md`
**Canonical output:** `sr-findings.md` (written after loop exits)
...
**Context file location:** `_bmad-output/sessions/{session_id}/planning/research-context-{story-key}.md`
...
```
research_context: _bmad-output/sessions/{session_id}/planning/research-context-{story-key}.md
story_key: {story-key}
is_ui_story: {is_ui_story}
```

**IR report location:** `_bmad-output/sessions/{session_id}/planning/story-ir-report-{story-key}.md` (also available for dev agent to review before starting implementation)
...
1. Create `_bmad-output/sessions/{session_id}/` directory
2. Initialize `_bmad-output/sessions/{session_id}/README.md` listing all stories being parallelized
...
Each agent MUST:
1. Write claims file FIRST to _bmad-output/sessions/{session_id}/claims-{story-slug}.md
2. Read ALL other claims files before touching any code
3. Follow .claude/agents/parallel-dev.md instructions exactly
4. Use context at _bmad-output/sessions/{session_id}/context.md
5. Write findings to _bmad-output/sessions/{session_id}/findings-{story-slug}.md
...
1. **Read all findings reports** from `_bmad-output/sessions/{session_id}/findings-*.md`
...
4. **If clean**, synthesize automatically:
   ...
   - Write synthesis summary to `_bmad-output/sessions/{session_id}/synthesis-{epic-slug}.md`:
...
5. **Report to user:**
   ...
   Synthesis report: _bmad-output/sessions/{session_id}/synthesis-{epic-slug}.md
